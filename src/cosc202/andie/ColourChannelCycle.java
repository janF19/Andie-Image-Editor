    ...
    public class CycleColours implements ImageOperation, java.io.Serializable {
        ...
        public BufferedImage apply(BufferedImage input) {
            ...
            for (int cycle = 1; cycle <= 2; cycle++) {
                BufferedImage cycleImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
                for (int y = 0; y < height; y++) {
                    for (int x = 0; x < width; x++) {
                        int rgb = input.getRGB(x, y);
                        int red = (rgb >> 16) & 0xFF;
        ...
        private void displayPreview(BufferedImage[] images, String[] titles) {
            ...
            for (int i = 1; i <= 2; i++) {
                ImageIcon icon = new ImageIcon(images[i].getScaledInstance(300, 400, Image.SCALE_DEFAULT));
                ...
                button.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        if (finalI != 0) {
                            ...
                            button.setIcon(new ImageIcon(images[0].getScaledInstance(300, 400, Image.SCALE_DEFAULT)));
